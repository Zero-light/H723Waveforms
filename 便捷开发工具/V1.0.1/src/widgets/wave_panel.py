...
class WavePanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.btn_cfg = QPushButton("发送配置")
        self.btn_cfg.clicked.connect(self._send_config)
        cfg_inner.addWidget(self.btn_cfg)
        ...
        for i, name in enumerate(CH_NAMES):
            ...
            if name == "SCLK":
                chk.setStyleSheet("QCheckBox { font-weight: bold; color: #e74c3c; }")
        ...
        self.btn_apply_rules = QPushButton("应用时序")
        self.btn_apply_rules.clicked.connect(self._apply_rules)
        btn_layout.addWidget(self.btn_apply_rules)
        ...
        # Buttons row: fill SCLK | export | import | load
        btn_row = QHBoxLayout()
        self.btn_fill_sclk = QPushButton("填充 SCLK")
        self.btn_fill_sclk.clicked.connect(self._fill_sclk)
        btn_row.addWidget(self.btn_fill_sclk)
        ...
        self.btn_load = QPushButton("加载模式到设备")
        self.btn_load.clicked.connect(self._send_data)
    ...
    def _warn_and_confirm(self, states, action_name="此操作"):
        ...
        safe, msg = self._check_waveform_safety(states, action_name)
        if safe:
            return True
    ...
    def _send_config(self):
        ...
        self.log_signal.emit(
            f"[TX] 波形配置 频率={self.sb_rate.value()}Hz "
            f"更新率={update_rate} 点数={self.sb_points.value()}"
        )
    ...
    def _send_data(self):
        ...
        if len(masks) > 0 and all(m == 0 for m in masks):
            QMessageBox.warning(self, "安全拦截",
                "BSRR 数组全为零，无法下发。\n"
                "请先在通道表格中添加至少一个高电平，或检查规则配置。")
            self.log_signal.emit("[SAFETY] 全零 BSRR 已拦截，未下发")
            return

        # Safety: per-channel long-low / all-low check before sending to device.
        states = self._get_channel_states()
        if not self._warn_and_confirm(states, "加载模式到设备"):
            self.log_signal.emit("[SAFETY] 用户取消下发高风险波形")
            return

        if len(masks) > 2048:
            QMessageBox.information(self, "提示",
                f"模式包含 {len(masks)} 个点；分块发送暂未实现。")
            return
        frame = build_wave_data(masks)
        self.link.send(frame.to_bytes())
        self.log_signal.emit(f"[TX] 波形数据 {len(masks)} 点")
    ...
    def _load_rules(self):
        ...
        if "rules" in data:
            for rule in data["rules"]:
                ...
                for name in CH_NAMES:
                    ch_combo.addItem(name)
                ch_combo.setCurrentText(rule.get("channel", "SCLK"))
                ch_combo.currentTextChanged.connect(self._save_rules)
                self.rule_table.setCellWidget(row, 0, ch_combo)
                edge_spin = QSpinBox()
                edge_spin.setRange(1, 4096)
