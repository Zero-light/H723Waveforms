    ...
    for (uint8_t ch = 0; ch < WAVE_PIN_COUNT; ch++) {
        if (ch_mask & (1u << ch)) {
            const BspGpioPin_t pin = {
                ...
                BSP_GPIO_PULL_NONE, BSP_GPIO_SPEED_VERY_HIGH, 0
            };
...
bool DRV_WaveGen_Configure(const DrvWaveConfig_t *cfg)
{
    ...
    s_config = *cfg;
    s_data_valid = false;
    return BSP_TIM_SetFrequency(s_timHandle, cfg->sample_rate_hz);
}
...
{
    if (s_running || s_config.num_points == 0 || !s_data_valid) return false;
    ...
    BSP_DMA_Start(s_dmaHandle, &dmaCfg);
    BSP_TIM_EnableDmaUpdate(s_timHandle);
    BSP_TIM_BaseStart(s_timHandle);
