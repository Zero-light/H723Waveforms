...
bool APP_Protocol_SendFrame(const Frame_t *frame)
{
    ...
    /* Critical section: s_txBuf must not be touched by ISR during assembly.
     * Once the frame is built, IRQs are re-enabled so the USB ISR can fire
     * and complete any previous transfer (clearing TxState). */
    ...
    /* Frame built — re-enable IRQs so USB completion ISR can fire.
     * Keep IRQs off only if the caller already had them off. */
    if (!primask) {
        __enable_irq();
    }

    /* Send with retry.  A previous CDC_Transmit_HS may still be in flight
     * (TxState != 0).  With IRQs enabled the USB ISR can complete it and
     * clear TxState.  Each retry burns ~15 us; 2000 attempts ≈ 30 ms max.
     * ponytail: this blocks the main loop during retries; at streaming rates
     * ≤ 50 kHz there is enough slack between batches that this rarely fires.
     * Upgrade path: queue-based async TX if higher rates are needed. */
    uint8_t ret = USBD_BUSY;
    for (int attempt = 0; attempt < 2000; attempt++) {
        ret = CDC_Transmit_HS(s_txBuf, pos);
        if (ret == USBD_OK) break;
        /* Brief spin to let USB ISR fire */
